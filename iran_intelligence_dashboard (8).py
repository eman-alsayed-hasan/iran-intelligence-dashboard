...
2026-03-03 13:42 UTC,AE023B,,40.6191,23.2666,32000,449,False,Souda Bay,573,United States
2026-03-03 13:42 UTC,AAB841,CKS702,39.1203,21.3987,26000,444,False,Souda Bay,502,United States
2026-03-03 13:42 UTC,ABF486,HAF403,36.5512,27.2135,11000,281,False,Souda Bay,358,United States
2026-03-03 13:42 UTC,AE63CB,GOLD16,36.6691,23.3869,30000,448,False,Souda Bay,152,United States
2026-03-03 13:42 UTC,A95442,N70X,34.4113,28.0419,37000,506,False,Souda Bay,450,United States
2026-03-03 13:42 UTC,AE03F9,DUKE73,40.5453,22.8356,1500,142,False,Souda Bay,575,United States
2026-03-03 13:42 UTC,ADFD70,CNV6202,36.6873,23.3205,24000,224,False,Souda Bay,158,United States
2026-03-03 13:42 UTC,AE023B,,40.6736,23.2631,32000,448,False,Souda Bay,579,United States
2026-03-03 13:42 UTC,AAB841,CKS702,39.1634,21.3572,26000,444,False,Souda Bay,509,United States
2026-03-03 13:42 UTC,ABF486,HAF403,36.5388,27.2530,11000,281,False,Souda Bay,362,United States
2026-03-03 13:42 UTC,AE63CB,GOLD16,36.7223,23.3758,30000,448,False,Souda Bay,158,United States
2026-03-03 13:42 UTC,A95442,N70X,34.3640,28.0912,37000,505,False,Souda Bay,456,United States
2026-03-03 13:42 UTC,AE03F9,DUKE73,40.5412,22.8572,1150,140,False,Souda Bay,574,United States
2026-03-03 13:42 UTC,ADFD70,CNV6202,36.7062,23.2979,24000,223,False,Souda Bay,161,United States
"""
...
READINESS_INDICATORS = [
    ...
    ("4x Patriot PAC-3 batteries active",          2, "Feb 22", "Reuters"),
...
def compute_sustainment_decay(
    ...
    sortie_multiplier = np.minimum(np.minimum(fuel_pct, ammo_pct), pers_pct) / 100
    ...
    resupply_windows = [
        supply_ship_interval_days * i
        for i in range(1, 4)
        if supply_ship_interval_days * i <= 30
    ]
...
def build_decay_chart(model):
    ...
    for thresh, label, col in [
        ...
        fig.add_hline(y=thresh, line_dash="dot", line_color=col, line_width=1,
                      ...
                      annotation_font=dict(color=col, size=8), row=1, col=1)
    ...
    fig.add_trace(go.Bar(
        y=["Fuel to 25%", "Ammo to 25%", "Sortie rate to 70%"],
        x=[model["fuel_critical_day"], model["ammo_critical_day"], model["sortie_degraded_day"]],
        orientation="h",
        marker_color=["#FF6600", "#FF2D2D", "#FFB300"],
        marker_line=dict(color="#0d1117", width=1),
        text=[f"Day {int(v)}" for v in [
    ...
    fig.update_layout(
        title=dict(
            text="<b>SUSTAINMENT DECAY MODEL — IRAN PERIMETER FORCE</b><br><sup>Countdown to Resupply | Based on published USAF/USN logistics doctrine | UNCLASSIFIED</sup>",
...
def build_readiness_chart():
    dates = pd.date_range("2026-01-01", "2026-02-28", freq="3D")
    ...
    for vals, name, col, alpha in [
        ...
        ([v*35 for v in mar_v],  "Maritime (x35)", "#00C8FF", "0.06"),
        ([v*20 for v in gnd_v],  "Ground (x20)",   "#00FF88", "0.06"),
    ]:
        r = int(col[1:3], 16)
        g = int(col[3:5], 16)
        b = int(col[5:7], 16)
    ...
    fig.update_layout(
        title=dict(text="<b>MULTI-INT BUILDUP CURVE — Jan 1 to Feb 28, 2026</b>",
...
def build_base_map():
    ...
    fig.update_geos(
        ...
        bgcolor="#0d1117", landcolor="#161b22", oceancolor="#0a0f16",
        countrycolor="#2a3040", showcoastlines=True, coastlinecolor="#2a3040",
...
if uploaded:
    ...
    data_source = f"Your file: {uploaded.name}"
else:
    df = pd.read_csv(io.StringIO(SAMPLE_HISTORY_CSV))
    data_source = "Live data: Mar 3, 2026 session | Mediterranean corridor | 7 aircraft"
...
Multi-INT Analysis | ADS-B + AIS + OSINT | Mar 3, 2026 | UNCLASSIFIED
</p>
""", unsafe_allow_html=True)
...
kpis = [
    ...
    (str(df["icao24"].nunique()),              "ADS-B Contacts",     "#A855F7"),
    (str(len(df[df.apply(lambda r: 230 <= float(r.get("speed_kts",0) or 0) <= 350 and float(r.get("altitude_ft",0) or 0) <= 18000 and str(r.get("on_ground","False")).lower()!="true", axis=1)])), "Orbit Contacts", "#FF6600"),
...
with tab3:
    ...
    if len(orbit_contacts) > 0:
        st.markdown("""
<div class='warning-box'>
<b>ORBIT CONTACTS DETECTED</b> — Contacts in orbit phase represent
...
""", unsafe_allow_html=True)
        for _, row in orbit_contacts.drop_duplicates(subset=["icao24"]).iterrows():
            st.markdown(
                f"**{row['callsign']}** ({row['icao24']}) — "
...
Ground and Maritime logistics account for 60% of total score.
This is not an air-only deployment.
</div>
""", unsafe_allow_html=True)
